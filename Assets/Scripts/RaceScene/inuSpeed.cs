...
public class inuSpeed : MonoBehaviour {
	...
	void Update ()
	{
		...
		//ここよりうえはレースが始まるまでレース開始位置に犬を配置



		if (RaceStart == true) {//信号が鳴りスタートしたら犬を歩かせる
	...
	private IEnumerator idou_Race () {
		...
		b=Random.Range(1.0f, 1.7f);
		Speed = b;
		rigidbody.velocity =new Vector3 (-Speed, 0, 0);
		animator.SetBool("walkRight",true);
		yield return null;
		/*yield return new WaitForSeconds (0.01f);
		a=PlayerPrefs.GetInt ("inutyoushi");
		Speed=Random.Range(a, 4.0f);
		rigidbody.velocity=new Vector3 (-Speed, 0, 0);
		animator.SetBool("walkRight",true);
		yield return null;*/
	}
