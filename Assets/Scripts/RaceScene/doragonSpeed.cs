...
public class doragonSpeed : MonoBehaviour {
	...
	void Update ()
	{
		...
		//ここよりうえはレースが始まるまでレース開始位置にドラゴンを配置



		if (RaceStart == true) {//信号が鳴りスタートしたら羊を歩かせる
	...
	private IEnumerator idou_Race () {
		yield return new WaitForSeconds (0.01f);
//		a=PlayerPrefs.GetInt ("doragontyoushi");
//		Speed = 1.0f*1/a;
		b=Random.Range(0.3f, 1.2f);
		Speed = b;
	...
	private IEnumerator Goal () {
		...
		textend.GetComponent<TextScript>().doragon = true;
		rigidbody.velocity =new Vector3 (0, 0, 0);
		animator.SetBool("walkRight",false);
		yield return null;
	}
